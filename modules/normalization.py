...
# IMPORTANT: rainfall range matches cnn_lstm.py (0-100 mm/hr)
INDICATOR_RANGES = {
    "rainfall":    (0, 100),  # FIXED: was (0,3), must match cnn_lstm.py
    "humidity":    (0, 100),
    "soil":        (0, 3),
    "flood":       (0, 1),
    "storm_surge": (0, 1),
...
def normalize(value: float, min_val: float, max_val: float) -> float:
    """Min-max normalization. Returns 0.0 if range is zero."""
    if max_val == min_val:
...
def compute_weighted_scores(
    ...
    raw_values = {
        ...
        "soil":        E.get("soil", 0),
        "flood":       E.get("flood", 0),
        "storm_surge": E.get("storm_surge", 0),
